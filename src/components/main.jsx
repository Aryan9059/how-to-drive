...
const App = () => {
  ...
  // Reset traffic violations when a new lesson session starts
  const resetTrafficViolations = useCallback(() => {
    simStore.trafficViolations = 0;
    simStore.metrics.trafficViolations = 0;
  }, []);
  ...
  const handleLessonPass = useCallback(() => {
    ...
    // Deduct stars for traffic violations beyond allowed
    const violationPenalty = Math.max(0, tlViolations - allowedViolations);
    earnedStars = Math.max(0, earnedStars - violationPenalty);
  ...
  // Traffic violation handler — called by TrafficLightController via Scene → Track
  const handleTrafficViolation = useCallback(({ state, id }) => {
    // simStore is already updated in TrafficLightController
    // Force a re-render of SimHUD by updating the reactive store copy
    // (SimHUD polls simStore every 80ms so it will pick it up automatically)
  }, []);
          ...
          <Canvas key={canvasKey} style={{ position: "fixed", inset: 0 }}>
            <Physics broadphase="SAP" gravity={getGravity()}>
              <Scene
                ...
                onTrafficViolation={handleTrafficViolation}
              />
            </Physics>
          </Canvas>
        </>
